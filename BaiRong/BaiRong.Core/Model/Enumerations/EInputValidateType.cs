...
namespace BaiRong.Model
{
	...
	public class EInputValidateTypeUtils
	{
        ...
        public static string GetValidateInfo(TableStyleInfo styleInfo)
        {
            ...
            {
                builder.AppendFormat("最少{0}个字符;", styleInfo.Additional.MinNum);
            }
            if (styleInfo.Additional.MaxNum > 0)
            {
                builder.AppendFormat("最多{0}个字符;", styleInfo.Additional.MaxNum);
            }
            if (styleInfo.Additional.ValidateType != EInputValidateType.None)
            {
                builder.AppendFormat("验证:{0};", EInputValidateTypeUtils.GetText(styleInfo.Additional.ValidateType));
            }
