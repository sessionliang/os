...
namespace BaiRong.Core.Net
{
    public class FTP
    {
        ...
        /// Constructor
        /// </summary>
        public FTP(string server, int port, string user, string pass, bool isPassiveMode)
        {
            this.server = server;
            this.user = user;
            this.pass = pass;
            this.port = port;
            passive_mode = isPassiveMode;		// #######################################
            main_sock = null;
        ...
        public void RemoveDirs(string dir)
        {
            ...
            foreach (string d in this.ListDirectories())
            {
                FTPFileSystemInfo subDirectoryInfo = new FTPFileSystemInfo(d);
                if (subDirectoryInfo.Name == "." || subDirectoryInfo.Name == "..") continue;
                this.RemoveDirs(subDirectoryInfo.Name);
            }

            foreach (string f in this.ListFiles())
            {
                FTPFileSystemInfo fileInfo = new FTPFileSystemInfo(f);
                this.RemoveFile(fileInfo.Name);
            }
