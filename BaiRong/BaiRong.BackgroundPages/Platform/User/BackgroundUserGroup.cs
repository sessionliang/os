...
namespace BaiRong.BackgroundPages
{
    public class BackgroundUserGroup : BackgroundBasePage
	{
        ...
        {
            return PageUtils.GetPlatformUrl(string.Format("background_userGroup.aspx?"));
        }
        ...
        public void Page_Load(object sender, EventArgs E)
        {
			...
			if (base.GetQueryString("Delete") != null)
			{
				...
				try
				{
                    string groupName = UserGroupManager.GetGroupName(string.Empty, groupID);
                    BaiRongDataProvider.UserGroupDAO.Delete(string.Empty, groupID);
			...
			if (!IsPostBack)
            {
				...
				BindGrid();
                this.AddButton1.Attributes.Add("onclick", Modal.UserGroupAdd.GetOpenWindowStringToAdd( 0, true));
                this.AddButton2.Attributes.Add("onclick", Modal.UserGroupAdd.GetOpenWindowStringToAdd( 0, false));
			}
		}
        ...
        public void BindGrid()
		{
            ...
            {
                BaiRongDataProvider.UserGroupDAO.CreateDefaultUserGroup(string.Empty);
                userGroupInfoArrayList = UserGroupManager.GetGroupInfoArrayList(string.Empty);
            }
        ...
        public void MyDataGrid1_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                ltlAddUrl.Text = string.Format(@"<a href='javascript:;' onclick=""{0}"">添加近似</a>", Modal.UserGroupAdd.GetOpenWindowStringToAdd( groupInfo.GroupID, true));
                ltlEditUrl.Text = string.Format(@"<a href='javascript:;' onclick=""{0}"">编辑</a>", Modal.UserGroupAdd.GetOpenWindowStringToEdit( groupInfo.GroupID, true));
                ...
                ltlDeleteUrl.Text = string.Format("<a href=\"{0}&Delete=True&GroupID={1}\" onClick=\"javascript:return confirm('此操作将删除用户组“{2}”，确认吗？');\">删除</a>", BackgroundUserGroup.GetRedirectUrl(), groupInfo.GroupID, groupInfo.GroupName);
            }
        }
        ...
        public void MyDataGrid2_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                ltlEditUrl.Text = string.Format(@"<a href='javascript:;' onclick=""{0}"">编辑</a>", Modal.UserGroupAdd.GetOpenWindowStringToEdit( groupInfo.GroupID, false));
