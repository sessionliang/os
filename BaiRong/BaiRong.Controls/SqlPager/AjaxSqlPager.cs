...
namespace BaiRong.Controls
{
    ...
    [ToolboxData("<{0}:AjaxSqlPager runat=\"server\" />")]
    public class AjaxSqlPager : Table, INamingContainer
    {
        ...
        private string GetQueryCountCommandText()
        {
            ...
            {
                return string.Format("SELECT COUNT(rowid) FROM ({0})", SelectCommand);
            }
        }
...
(
    SELECT * FROM (
        ...
        public string AjaxScriptName
        {
            get { return Convert.ToString(ViewState["AjaxScriptName"]); }
            set { ViewState["AjaxScriptName"] = value; }
        }
        ...
        {
            CurrentPageIndex = TranslateUtils.ToInt(Page.Request.QueryString[AjaxSqlPager.PARM_PAGE], 1) - 1;
        ...
        private string GetNavigationUrl(int page)
        {
            ...
            if (page > 1)
            {
                queryString[AjaxSqlPager.PARM_PAGE] = page.ToString();
            }
            else
            {
                queryString.Remove(AjaxSqlPager.PARM_PAGE);
            }
            ...
            return PageUtils.AddProtocolToUrl(PageUtils.AddQueryString(PageUtils.GetUrlWithoutQueryString(Page.Request.RawUrl), queryString));
        }
        ...
        private void BuildNextPrevUI(TableCell cell)
        {
            ...
            if (enabled)
            {
                ...
                link.NavigateUrl = PageUtils.UNCLICKED_URL;
                link.Attributes.Add("onclick", string.Format("{0}('{1}')", this.AjaxScriptName, this.GetNavigationUrl(1)));
            ...
            if (enabled)
            {
                ...
                link.Style.Add("text-decoration", "none");
                link.NavigateUrl = PageUtils.UNCLICKED_URL;
                link.Attributes.Add("onclick", string.Format("{0}('{1}')", this.AjaxScriptName, this.GetNavigationUrl(this.CurrentPageIndex)));
                if (!string.IsNullOrEmpty(this.EnabledCssClass))
                {
                    link.CssClass = this.EnabledCssClass;
                }
            ...
            if (enabled)
            {
                ...
                link.Style.Add("text-decoration", "none");
                link.NavigateUrl = PageUtils.UNCLICKED_URL;
                link.Attributes.Add("onclick", string.Format("{0}('{1}')", this.AjaxScriptName, this.GetNavigationUrl(this.CurrentPageIndex + 2)));
                if (!string.IsNullOrEmpty(this.EnabledCssClass))
                {
                    link.CssClass = this.EnabledCssClass;
                }
            ...
            if (enabled)
            {
                ...
                link.Style.Add("text-decoration", "none");
                link.NavigateUrl = PageUtils.UNCLICKED_URL;
                link.Attributes.Add("onclick", string.Format("{0}('{1}')", this.AjaxScriptName, this.GetNavigationUrl(this.TotalPages)));
                if (!string.IsNullOrEmpty(this.EnabledCssClass))
                {
                    link.CssClass = this.EnabledCssClass;
                }
        ...
        private void BuildCurrentPage(TableCell cell)
        {
            ...
            HtmlSelect pageList = new HtmlSelect();
            ...
            if (TotalPages <= 0 || CurrentPageIndex == -1)
            {
                ...
                pageList.Attributes.Add("enabled", "false");
            ...
            else // Populate the list
            {
                for (int i = 1; i <= TotalPages; i++)
                {
                    ...
                    string navigationUrl = this.GetNavigationUrl(i);
                    item.Attributes.Add("navigationUrl", navigationUrl);
                    pageList.Items.Add(item);
                }
                pageList.SelectedIndex = CurrentPageIndex;

                pageList.Attributes.Add("onchange", string.Format("{0}(this.options[this.selectedIndex]['navigationUrl'])", this.AjaxScriptName));
            }
