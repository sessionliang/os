...
namespace BaiRong.Controls
{
    ...
    [ToolboxData("<{0}:SqlPagerByDataReader runat=\"server\" />")]
    public class SqlPagerByDataReader : Table, INamingContainer
    {
    ...
    SELECT * FROM (
        ...
        private void FetchAllData()
        {
            ...
            if (data == null)
            {
                ...
                using (IDbConnection conn = SqlUtils.GetIDbConnection(BaiRongDataProvider.ADOType, ConnectionString))
                {
                    conn.Open();
                    ...
                    IDataReader reader = cmd.ExecuteReader();   /*创建一个reader*/

                    data.Load(reader);     /*就这一句用DataTable.Load(DataReader)来把查询到的数据插入到DataTable中*/
        ...
        private void FetchPageData()
        {
            ...
            using (IDbConnection conn = SqlUtils.GetIDbConnection(BaiRongDataProvider.ADOType, ConnectionString))
            {
                ...
                TSelectReSoult.Load(reader);     /*就这一句用DataTable.Load(DataReader)来把查询到的数据插入到DataTable中*/
                ...
                _dataSource.DataSource = TSelectReSoult.DefaultView;
        }
