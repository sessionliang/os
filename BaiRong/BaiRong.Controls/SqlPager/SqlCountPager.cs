...
(
    SELECT * FROM (
        ...
        {
            CurrentPageIndex = TranslateUtils.ToInt(Page.Request.QueryString[SqlCountPager.PARM_PAGE], 1) - 1;
        ...
        private string GetNavigationUrl(int page)
        {
            ...
            if (page > 1)
            {
                queryString[SqlCountPager.PARM_PAGE] = page.ToString();
            }
            else
            {
                queryString.Remove(SqlCountPager.PARM_PAGE);
            }
        ...
        private void BuildNextPrevUI(TableCell cell)
        {
            ...
            if (TotalPages > 0)
            {
                ...
                cell.Controls.Add(new LiteralControl("&nbsp;&nbsp;"));
                Label totalPagesText = new Label();
                lastText.Text = string.Format(@" ( 共{0}条 )", TotalCount);
                cell.Controls.Add(totalPagesText);
                //cell.Controls.Add(new LiteralControl(string.Format(@"<div class=""Results""> ( 共{0}条 {1})</div>", TotalCount, this.Unit)));
            }
        }
