		...
		public string GetItemHtml()
		{
            ...
            if (this.enabled)
            {
                if (!string.IsNullOrEmpty(this.linkUrl))
                {
                    ...
                    if (openWindow)
                    {
                        htmlBuilder.AppendFormat(@"&nbsp;<a href='{0}' target='_blank' title='新窗口打开'><img src=""{1}"" /></a>", this.linkUrl, PageUtils.ParseNavigationUrl("~/sitefiles/bairong/icons/tree/open.png"));
                    }
                }
