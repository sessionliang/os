...
function fnIsReceive(obj, fieID, userName, isRecevice) {
    $.getJSON("ajax/getAjax.aspx?", { FileID: fieID, UserName: userName, IsRecevice: isRecevice }, function (data) {
        if (data.IsRecevice == "true" && data.HasData == "true") {
            $("#xwb_update_tips").show();
            $(obj).parent().parent().children("td").eq(0).hide();
            $(obj).parent().parent().children("td").eq(1).show();
        }
        else if (data.IsRecevice == "false" && data.HasData == "true") {
            $("#xwb_update_tips").show();
            $(obj).parent().parent().remove();
        }
        else if ((data.IsRecevice == "true" && data.HasData == "false") || (data.IsRecevice == "false" && data.HasData == "false")) {
            $("#xwb_update_tips").fadeOut("slow");
        }
