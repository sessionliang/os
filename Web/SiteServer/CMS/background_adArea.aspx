<%@ Page Language="C#" Inherits="SiteServer.CMS.BackgroundPages.BackgroundAdArea" %>
...
<form id="Form1" class="form-inline" runat="server">
  ...
  <asp:dataGrid id="dgContents" showHeader="true" AutoGenerateColumns="false" DataKeyField="AdAreaName" HeaderStyle-CssClass="info thead" CssClass="table table-bordered table-hover" gridlines="none" runat="server">
    <Columns>
      ...
      <asp:TemplateColumn >
        <ItemTemplate> <a href="background_adAreaAdd.aspx?PublishmentSystemID=<%=Request.QueryString["PublishmentSystemID"]%>&AdAreaName=<%# DataBinder.Eval(Container.DataItem,"AdAreaName")%>">编辑</a> </ItemTemplate>
        <ItemStyle Width="50" cssClass="center" />
      </asp:TemplateColumn>
      <asp:TemplateColumn >
        <ItemTemplate> <a href="background_adv.aspx?PublishmentSystemID=<%=Request.QueryString["PublishmentSystemID"]%>&AdAreaID=<%# DataBinder.Eval(Container.DataItem,"AdAreaID")%>">广告</a> </ItemTemplate>
        <ItemStyle Width="50" cssClass="center" />
      </asp:TemplateColumn>
      <asp:TemplateColumn >
        <ItemTemplate> <a href="background_adArea.aspx?PublishmentSystemID=<%=Request.QueryString["PublishmentSystemID"]%>&AdAreaName=<%# DataBinder.Eval(Container.DataItem,"AdAreaName")%>&Delete=True" onClick="javascript:return confirm('此操作将删除广告位“<%# DataBinder.Eval(Container.DataItem,"AdAreaName")%>”，确认吗？');">删除</a> </ItemTemplate>
        <ItemStyle Width="50" cssClass="center" />
      </asp:TemplateColumn>
      <asp:TemplateColumn
