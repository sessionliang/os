<%@ Page Language="C#" Inherits="SiteServer.CMS.BackgroundPages.BackgroundAdv" %>
...
<form id="Form1" class="form-inline" runat="server">
  ...
  <asp:dataGrid id="dgContents" showHeader="true" AutoGenerateColumns="false" DataKeyField="AdvID" HeaderStyle-CssClass="info thead" CssClass="table table-bordered table-hover" gridlines="none" runat="server">
    <Columns>
      ...
      <asp:TemplateColumn >
        <ItemTemplate> <a href="background_advAdd.aspx?PublishmentSystemID=<%=Request.QueryString["PublishmentSystemID"]%>&AdvID=<%# DataBinder.Eval(Container.DataItem,"AdvID")%>">编辑</a> </ItemTemplate>
        <ItemStyle Width="50" cssClass="center" />
      </asp:TemplateColumn>
       <asp:TemplateColumn >
        <ItemTemplate> <a href="background_adMaterial.aspx?PublishmentSystemID=<%=Request.QueryString["PublishmentSystemID"]%>&AdvID=<%# DataBinder.Eval(Container.DataItem,"AdvID")%>">广告物料</a> </ItemTemplate>
        <ItemStyle Width="50" cssClass="center" />
      </asp:TemplateColumn>  
      <asp:TemplateColumn >
        <ItemTemplate> <a href="background_adv.aspx?PublishmentSystemID=<%=Request.QueryString["PublishmentSystemID"]%>&AdvID=<%# DataBinder.Eval(Container.DataItem,"AdvID")%>&Delete=True" onClick="javascript:return confirm('此操作将删除广告“<%# DataBinder.Eval(Container.DataItem,"AdvName")%>”，确认吗？');">删除</a> </ItemTemplate>
        <ItemStyle Width="50" cssClass="center" />
      </asp:TemplateColumn>
      <asp:TemplateColumn
