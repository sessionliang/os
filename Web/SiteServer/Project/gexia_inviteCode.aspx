<%@ Page Language="C#" Inherits="SiteServer.GeXia.BackgroundPages.BackgroundInviteCode" %>

<%@ Register TagPrefix="bairong" Namespace="BaiRong.Controls" Assembly="BaiRong.Controls" %>

<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8">
<!--#include file="../inc/header.aspx"-->
</head>

<body>
<!--#include file="../inc/openWindow.html"-->
<form class="form-inline" runat="server">
  <asp:Literal id="ltlBreadCrumb" runat="server" />
  <bairong:alerts runat="server" />

  <script type="text/javascript">
  $(document).ready(function()
  {
    loopRows(document.getElementById('contents'), function(cur){ cur.onclick = chkSelect; });
    $(".popover-hover").popover({trigger:'hover',html:true});
  });
  </script>

  <div class="well well-small">
    领取量：<code><asp:Literal ID="ltlHoldCount" runat="server"></asp:Literal></code>
    使用量：<code><asp:Literal ID="ltlCashCount" runat="server"></asp:Literal></code>
    剩余量：<code><asp:Literal ID="ltlAvaliableCount" runat="server"></asp:Literal></code>
  </div>

  <table id="contents" class="table table-bordered table-hover">
    <tr class="info thead">
      <td width="20"></td>
      <td>SN码</td>
      <td>状态</td>
      <td>领取时间</td>
      <td>领取人申请信息</td>
      <td>使用时间</td>
      <td>使用者</td>
      <td width="20"><input type="checkbox" onClick="selectRows(document.getElementById('contents'), this.checked);" /></td>
    </tr>
    <asp:Repeater ID="rptContents" runat="server">
      <itemtemplate>
        <tr>
          <td class="center">
            <asp:Literal ID="ltlItemIndex" runat="server"></asp:Literal>
          </td>
          <td>
            <asp:Literal ID="ltlSN" runat="server"></asp:Literal>
          </td>
          <td class="center">
            <asp:Literal ID="ltlStatus" runat="server"></asp:Literal>
          </td>
          <td class="center">
            <asp:Literal ID="ltlHoldDate" runat="server"></asp:Literal>
          </td>
          <td class="center">
            <asp:Literal ID="ltlHoldApplication" runat="server"></asp:Literal>
          </td>
          <td class="center">
            <asp:Literal ID="ltlCashDate" runat="server"></asp:Literal>
          </td>
           <td class="center">
            <asp:Literal ID="ltlCashUserName" runat="server"></asp:Literal>
          </td>
          <td class="center">
            <input type="checkbox" name="IDCollection" value='<%#DataBinder.Eval(Container.DataItem, "ID")%>' />
          </td>
        </tr>
      </itemtemplate>
    </asp:Repeater>
  </table>

  <bairong:sqlPager id="spContents" runat="server" class="table table-pager" />

  <ul class="breadcrumb breadcrumb-button">
    <asp:Button class="btn btn-success" id="btnAdd" Text="添 加" runat="server" />
    <asp:Button class="btn" id="btnSetting" Text="设置状态" runat="server" />
    <asp:Button class="btn" id="btnReturn" Text="返 回" runat="server" />
  </ul>

</form>
</body>
</html>