...
namespace SiteServer.Project.Core
{
    public class UEditorUploader
    {
      ...
      * @return Hashtable
      */
        public Hashtable upFile(HttpContext cxt, int projectID)
        {
            try
            {
                ...
                string localDirectoryPath = ProjectManager.GetDirectoryUploadPath(projectID);
                string localFileName = PathUtils.GetFileName(originalName);
                string localFilePath = PathUtils.Combine(localDirectoryPath, localFileName);
                string fileExtName = PathUtils.GetExtension(originalName);
                ...
                if (state == "SUCCESS")
                {
                    ...
                    URL = PageUtils.Combine(ProjectManager.GetDirectoryUploadUrl(projectID), localFileName);
        ...
        public Hashtable upScrawl(HttpContext cxt, string base64Data, int projectID)
        {
            FileStream fs = null;
            try
            {
                ...
                URL = PageUtils.Combine(ProjectManager.GetDirectoryUploadUrl(projectID), fileName);
            }
            catch (Exception e)
            {
                state = "未知错误";
