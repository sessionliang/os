    ...
    public class BackgroundInvoice : BackgroundBasePage
    {
        ...
        public void Page_Load(object sender, EventArgs E)
        {
            if (!string.IsNullOrEmpty(base.Request.QueryString["Delete"]))
            {
                ...
                if (arraylist.Count > 0)
                {
                    try
                    {
                        ArrayList orderIDArrayList = DataProvider.InvoiceDAO.GetOrderIDArrayList(arraylist);
                        DataProvider.OrderDAO.UpdateIsInvoice(orderIDArrayList, false);
                        DataProvider.InvoiceDAO.Delete(arraylist);
        ...
        void rptContents_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                ltlInvoiceType.Text = EInvoiceTypeUtils.GetText(invoiceInfo.InvoiceType);
