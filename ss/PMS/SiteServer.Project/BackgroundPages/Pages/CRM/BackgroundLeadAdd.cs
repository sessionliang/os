...
namespace SiteServer.Project.BackgroundPages
{
    public class BackgroundLeadAdd : BackgroundBasePage
    {
        ...
        public string GetOptions(string attributeName)
        {
            ...
            {
                if (this.leadInfo == null || this.leadInfo.Status == ELeadStatus.New || this.leadInfo.Status == ELeadStatus.Contacting)
                {
                ...
                {
                    builder.AppendFormat(@"<option {0} value=""{1}"">{2}</option>", base.GetSelected(attributeName, ELeadStatusUtils.GetValue(this.leadInfo.Status)), ELeadStatusUtils.GetValue(this.leadInfo.Status), ELeadStatusUtils.GetText(this.leadInfo.Status));
                }
            }
        ...
        public void Page_Load(object sender, EventArgs E)
        {
            ...
            if (id > 0)
            {
                ...
                base.AddAttributes(this.leadInfo);
        ...
        public override void Submit_OnClick(object sender, System.EventArgs e)
        {
            try
            {
                ...
                if (this.leadInfo != null)
                {
                    LeadInfo leadInfoToEdit = DataProvider.LeadDAO.GetLeadInfo(AdminManager.Current.UserName, base.Request.Form);
                    foreach (string attributeName in LeadAttribute.BasicAttributes)
                    {
                        if (base.Request.Form[attributeName] != null)
                        {
                            this.leadInfo.SetExtendedAttribute(attributeName, leadInfoToEdit.GetExtendedAttribute(attributeName));
                        }
                    }
                ...
                {
                    LeadInfo leadInfoToAdd = DataProvider.LeadDAO.GetLeadInfo(AdminManager.Current.UserName, base.Request.Form);
                    if (!string.IsNullOrEmpty(location) && location.Split(',').Length == 3)
                    {
        ...
        {
            AccountInfo accountInfo = DataProvider.AccountDAO.GetAccountInfo(AdminManager.Current.UserName, base.Request.Form);
            if (!string.IsNullOrEmpty(location) && location.Split(',').Length == 3)
            {
        ...
        private void AccountUpdate(string location)
        {
            if (this.leadInfo.IsAccount)
            {
                ...
                if (accountInfo != null)
                {
                    ...
                    {
                        accountInfo.SetExtendedAttribute(attributeName, accountInfoToEdit.GetExtendedAttribute(attributeName));
                    }
                    if (!string.IsNullOrEmpty(location) && location.Split(',').Length == 3)
                    {
                        accountInfo.Province = location.Split(',')[0];
