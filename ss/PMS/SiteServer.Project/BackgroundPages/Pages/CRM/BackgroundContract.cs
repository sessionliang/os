    ...
    public class BackgroundContract : BackgroundBasePage
    {
        ...
        public void Page_Load(object sender, EventArgs E)
        {
            if (!string.IsNullOrEmpty(base.Request.QueryString["Delete"]))
            {
                ...
                if (arraylist.Count > 0)
                {
                    try
                    {
                        ArrayList orderIDArrayList = DataProvider.ContractDAO.GetOrderIDArrayList(arraylist);
                        DataProvider.OrderDAO.UpdateIsContract(orderIDArrayList, false);
                        DataProvider.ContractDAO.Delete(arraylist);
        ...
        void rptContents_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                ltlContractType.Text = EContractTypeUtils.GetText(contractInfo.ContractType);
