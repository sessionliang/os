...
{
	public class BackgroundTemplateJsAdd : BackgroundBasePage
	{
		...
		public void Page_Load(object sender, EventArgs E)
        {
			...
			if (!IsPostBack)
			{
                string pageTitle = string.IsNullOrEmpty(this.fileName) ? "添加脚本文件" : "编辑脚本文件";
				...
				{
                    if (!EFileSystemTypeUtils.IsJS(PathUtils.GetExtension(this.fileName)))
                    {
                ...
                {
                    this.ltlCreatedFileExtName.Text = ".js";
				...
				if (this.fileName != null)
				{
                    ...
                    if (PathUtils.RemoveExtension(this.fileName) != PathUtils.RemoveExtension(RelatedFileName.Text))//文件名改变
                    {
                        ...
                        foreach (string theFileName in fileNames)
                        {
                            ...
                            {
                                base.FailMessage("脚本文件修改失败，脚本文件已存在！");
                                return;
                            }
                        }
					...
					try
					{
                        ...
                        StringUtility.AddLog(base.PublishmentSystemID, "修改脚本文件", string.Format("脚本文件:{0}", currentFileName));
						base.SuccessMessage("脚本文件修改成功！");
                        base.AddWaitAndRedirectScript(PageUtils.GetSTLUrl(string.Format("background_templateJs.aspx?PublishmentSystemID={0}", base.PublishmentSystemID)));
					}
					catch(Exception ex)
					{
						base.FailMessage(ex, "脚本文件修改失败," + ex.Message);
					}
				}
				else
				{
                    ...
                    foreach (string theFileName in fileNames)
                    {
                        ...
                        {
                            base.FailMessage("脚本文件添加失败，脚本文件文件已存在！");
                            return;
                        }
                    }
					...
					{
                        FileUtils.WriteText(PathUtils.Combine(this.directoryPath, currentFileName), charset, this.Content.Text);
                        StringUtility.AddLog(base.PublishmentSystemID, "添加脚本文件", string.Format("脚本文件:{0}", currentFileName));
						base.SuccessMessage("脚本文件添加成功！");
                        base.AddWaitAndRedirectScript(PageUtils.GetSTLUrl(string.Format("background_templateJs.aspx?PublishmentSystemID={0}", base.PublishmentSystemID)));
					}
					catch(Exception ex)
					{
                        base.FailMessage(ex, "脚本文件添加失败," + ex.Message);
					}
				}
			}
		}
