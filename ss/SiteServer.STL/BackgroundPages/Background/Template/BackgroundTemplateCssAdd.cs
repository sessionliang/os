...
{
	public class BackgroundTemplateCssAdd : BackgroundBasePage
	{
		...
		public void Page_Load(object sender, EventArgs E)
        {
            ...
            this.directoryPath = PathUtility.MapPath(base.PublishmentSystemInfo, "@/css");

			if (!IsPostBack)
			{
                string pageTitle = string.IsNullOrEmpty(this.fileName) ? "添加样式文件" : "编辑样式文件";
				...
				{
                    if (!EFileSystemTypeUtils.IsCSS(PathUtils.GetExtension(this.fileName)))
                    {
                ...
                {
                    this.ltlCreatedFileExtName.Text = ".css";
				...
				if (this.fileName != null)
				{
                    ...
                    if (PathUtils.RemoveExtension(this.fileName) != PathUtils.RemoveExtension(RelatedFileName.Text))//文件名改变
                    {
                        ...
                        foreach (string theFileName in fileNames)
                        {
                            ...
                            {
                                base.FailMessage("样式文件修改失败，样式文件已存在！");
                                return;
                            }
                        }
					...
					try
					{
                        ...
                        StringUtility.AddLog(base.PublishmentSystemID, "修改样式文件", string.Format("样式文件:{0}", currentFileName));
						base.SuccessMessage("样式文件修改成功！");
                        base.AddWaitAndRedirectScript(PageUtils.GetSTLUrl(string.Format("background_templateCss.aspx?PublishmentSystemID={0}", base.PublishmentSystemID)));
					}
					catch(Exception ex)
					{
						base.FailMessage(ex, "样式文件修改失败," + ex.Message);
					}
				}
				else
				{
                    ...
                    foreach (string theFileName in fileNames)
                    {
                        ...
                        {
                            base.FailMessage("样式文件添加失败，样式文件文件已存在！");
                            return;
                        }
                    }
					...
					{
                        FileUtils.WriteText(PathUtils.Combine(this.directoryPath, currentFileName), charset, this.Content.Text);
                        StringUtility.AddLog(base.PublishmentSystemID, "添加样式文件", string.Format("样式文件:{0}", currentFileName));
						base.SuccessMessage("样式文件添加成功！");
                        base.AddWaitAndRedirectScript(PageUtils.GetSTLUrl(string.Format("background_templateCss.aspx?PublishmentSystemID={0}", base.PublishmentSystemID)));
					}
					catch(Exception ex)
					{
                        base.FailMessage(ex, "样式文件添加失败," + ex.Message);
					}
				}
			}
		}
