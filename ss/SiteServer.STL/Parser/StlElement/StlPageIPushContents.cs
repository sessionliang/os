...
namespace SiteServer.STL.Parser.StlElement
{
    ...
    /// 增加智能推送内容列表分页标签
    /// </summary>
    public class StlPageIPushContents : StlIPushContents
    {
        public new const string ElementName = "stl:pageipushcontents";//可翻页内容列表
        ...
        public new static ListDictionary AttributeList
        {
            get
            {
                ListDictionary attributes = StlIPushContents.AttributeList;
                attributes.Add(Attribute_PageNum, "每页显示的内容数目");
                return attributes;
            }
        }

        public StlPageIPushContents(string stlPageContentsElement, PageInfo pageInfo, ContextInfo contextInfo, bool isXmlContent)
        {
        ...
        //InnerSearchOutput调用
        public StlPageIPushContents(string stlPageContentsElement, PageInfo pageInfo, ContextInfo contextInfo, int pageNum, string whereString)
        {
            this.pageInfo = pageInfo;
            this.contextInfo = contextInfo;
...
function stlDynamicPage(pageNum)
{{
    try
    {{
        if(pageNum=='nextPage'){{
