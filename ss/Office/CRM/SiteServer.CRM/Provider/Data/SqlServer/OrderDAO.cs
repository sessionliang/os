...
namespace SiteServer.CRM.Provider.Data.SqlServer
{
    public class OrderDAO : DataProviderBase, IOrderDAO
	{
        public int Insert(OrderInfo orderInfo)
        {
            ...
            string SQL_INSERT = BaiRongDataProvider.TableStructureDAO.GetInsertSqlString(orderInfo.Attributes, OrderInfo.TableName, out parms);
        ...
        public void Update(OrderInfo orderInfo)
        {
            orderInfo.BeforeExecuteNonQuery();
            IDbDataParameter[] parms = null;
            string SQL_UPDATE = BaiRongDataProvider.TableStructureDAO.GetUpdateSqlString(orderInfo.Attributes, OrderInfo.TableName, out parms);
        ...
        public void UpdateIsInvoice(List<int> orderIDList, bool isInvoice)
        {
            if (orderIDList != null && orderIDList.Count > 0)
            {
                string sqlString = string.Format("UPDATE {0} SET {1} = '{2}' WHERE ID IN ({3})", OrderInfo.TableName, OrderAttribute.IsInvoice, isInvoice.ToString(), TranslateUtils.ObjectCollectionToSqlInStringWithoutQuote(orderIDList));
                this.ExecuteNonQuery(sqlString);
            }
        }
        ...
        public void UpdateIsContract(List<int> orderIDList, bool isContract)
        {
            if (orderIDList != null && orderIDList.Count > 0)
            {
                string sqlString = string.Format("UPDATE {0} SET {1} = '{2}' WHERE ID IN ({3})", OrderInfo.TableName, OrderAttribute.IsContract, isContract.ToString(), TranslateUtils.ObjectCollectionToSqlInStringWithoutQuote(orderIDList));
                this.ExecuteNonQuery(sqlString);
            }
        }
        ...
        public OrderInfo GetOrderInfo(int orderID)
        {
            ...
            string SQL_SELECT = BaiRongDataProvider.TableStructureDAO.GetSelectSqlString(OrderInfo.TableName, 0, SqlUtils.Asterisk, SQL_WHERE, null);
        ...
        public string GetSelectString(EOrderType orderType)
        {
            ...
            return BaiRongDataProvider.TableStructureDAO.GetSelectSqlString(OrderInfo.TableName, 0, SqlUtils.Asterisk, whereString, null);
        }
