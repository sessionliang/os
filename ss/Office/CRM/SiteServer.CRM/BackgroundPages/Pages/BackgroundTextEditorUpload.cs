    ...
    public class BackgroundTextEditorUpload : BackgroundBasePage
    {
            ...
            if (!string.IsNullOrEmpty(base.Request.QueryString["CKEditor"]))
            {
                ...
                if (base.Request.Files != null && base.Request.Files.Count >= 1)
                {
                    ...
                    if (myFile != null && "" != myFile.FileName)
                    {
                        ...
                        try
                        {
                            string directoryPath = ProjectManager.GetDirectoryUploadPath(base.PublishmentSystemID);
                            ...
                            string imageUrl = PageUtils.Combine(ProjectManager.GetDirectoryUploadUrl(base.PublishmentSystemID), fileName);
        ...
        private void UploadForUEditor()
        {
            if (this.fileType == "Image")
            {
                ...
                info = up.upFile(HttpContext.Current, base.PublishmentSystemID);                               //获取上传状态
            ...
            else if (this.fileType == "Scrawl")
            {
                ...
                else
                {
                    Hashtable info = new Hashtable();
                    UEditorUploader up = new UEditorUploader();
                    info = up.upScrawl(HttpContext.Current, base.Request["content"], base.PublishmentSystemID); //获取上传状态
                ...
                if (action == "get")
                {
                    ...
                    if (DirectoryUtils.IsDirectoryExists(directoryPath))
                    {
                        ...
                        foreach (string filePath in filePathArray)
                        {
                            ...
                            {
                                str += PageUtils.Combine(ProjectManager.GetDirectoryUploadUrl(base.PublishmentSystemID), PathUtils.GetFileName(filePath)) + "ue_separate_ue";
