    ...
    public class BackgroundLeadAll : BackgroundBasePage
    {
        ...
        public void Page_Load(object sender, EventArgs E)
        {
            if (!string.IsNullOrEmpty(base.Request.QueryString["Delete"]))
            {
                ...
                if (deleteIDList.Count > 0)
                {
                    try
                    {
                        DataProvider.LeadDAO.Delete(deleteIDList);
        ...
        void rptContents_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                ELeadStatus status = ELeadStatusUtils.GetEnumType(leadInfo.Status);
                ltlStatus.Text = ELeadStatusUtils.GetText(status);
                if (status == ELeadStatus.New)
                {
                    ltlStatus.Text = string.Format("<span style='color:red'>{0}</span>", ltlStatus.Text);
                }
                else if (status == ELeadStatus.Success)
                {
