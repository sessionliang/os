...
namespace SiteServer.CRM.BackgroundPages
{
    public class BackgroundInvoiceAdd : BackgroundBasePage
    {
        ...
        public void Page_Load(object sender, EventArgs E)
        {
            ...
            if (this.invoiceInfo != null)
            {
                base.AddAttributes(this.invoiceInfo.ToNameValueCollection());
        ...
        public override void Submit_OnClick(object sender, System.EventArgs e)
        {
            try
            {
                if (this.invoiceInfo != null)
                {
                    InvoiceInfo invoiceInfoToEdit = new InvoiceInfo(base.Request.Form);
                    foreach (string attributeName in InvoiceAttribute.AllAttributes)
                    {
                        if (base.Request.Form[attributeName] != null)
                        {
                            this.invoiceInfo.SetValue(attributeName, invoiceInfoToEdit.GetValue(attributeName));
                        }
                    }

                    if (this.orderInfo != null)
                    {
                        this.invoiceInfo.InvoiceType = EInvoiceTypeUtils.GetValue(EInvoiceType.SiteYun);
                        this.invoiceInfo.OrderID = this.orderInfo.ID;
                    }
                    else if (this.accountInfo != null)
                    {
                        this.invoiceInfo.InvoiceType = EInvoiceTypeUtils.GetValue(EInvoiceType.SiteServer);
                ...
                {
                    InvoiceInfo invoiceInfoToAdd = new InvoiceInfo(base.Request.Form);

                    if (this.orderInfo != null)
                    {
                        invoiceInfoToAdd.InvoiceType = EInvoiceTypeUtils.GetValue(EInvoiceType.SiteYun);
                        invoiceInfoToAdd.OrderID = this.orderInfo.ID;
                    }
                    else if (this.accountInfo != null)
                    {
                        invoiceInfoToAdd.InvoiceType = EInvoiceTypeUtils.GetValue(EInvoiceType.SiteServer);
