...
{
    public class BackgroundOrderReturnList1 : BackgroundBasePage
    {
        ...
        public HyperLink hlReturnOrderStatusPass;
        public HyperLink hlReturnOrderStatusUnPass;
        public HyperLink hlReturnMoneyStatusPass;
        public HyperLink hlReturnMoneyStatusUnPass;
        ...
        public void Page_Load(object sender, EventArgs E)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (base.GetQueryString("ReturnOrder") != null)
                {

                    List<int> idList = TranslateUtils.StringCollectionToIntList(base.GetQueryString("IDCollection"));
                    if (base.GetQueryString("ReturnOrder") == "True")
                    {
                        try
                        {
                            DataProviderB2C.OrderItemReturnDAO.UpdateStatus(idList, new OrderItemReturnStatus()
                            {
                                ReturnOrderDate = DateTime.Now,
                                ReturnOrderUser = BaiRongDataProvider.AdministratorDAO.UserName,
                                ReturnOrderStatus = EReturnOrderStatusUtils.GetValue(EReturnOrderStatus.Received),
                                Status = EStatusUtils.GetValue(EStatus.Doing)
                            });

                            base.SuccessMessage("状态修改成功！");
                        }
                        catch (Exception ex)
                        {
                    ...
                    else
                    {
                        try
                        {
                            DataProviderB2C.OrderItemReturnDAO.UpdateStatus(idList, new OrderItemReturnStatus()
                            {
                                ReturnOrderDate = DateTime.Now,
                                ReturnOrderUser = BaiRongDataProvider.AdministratorDAO.UserName,
                                ReturnOrderStatus = EReturnOrderStatusUtils.GetValue(EReturnOrderStatus.Nrcy),
                ...
                else if (base.GetQueryString("ReturnMoney") != null)
                {

                    List<int> idList = TranslateUtils.StringCollectionToIntList(base.GetQueryString("IDCollection"));
                    if (base.GetQueryString("ReturnMoney") == "True")
                    {
                        try
                        {
                            DataProviderB2C.OrderItemReturnDAO.UpdateStatus(idList, new OrderItemReturnStatus()
                            {
                                ReturnMoneyDate = DateTime.Now,
                                ReturnMoneyUser = BaiRongDataProvider.AdministratorDAO.UserName,
                                ReturnMoneyStatus = EReturnMoneyStatusUtils.GetValue(EReturnMoneyStatus.Refund),
                                Status = EStatusUtils.GetValue(EStatus.Done)
                            });
                    ...
                    else
                    {
                        try
                        {
                            DataProviderB2C.OrderItemReturnDAO.UpdateStatus(idList, new OrderItemReturnStatus()
                            {
                                ReturnMoneyDate = DateTime.Now,
                                ReturnMoneyUser = BaiRongDataProvider.AdministratorDAO.UserName,
                                ReturnMoneyStatus = EReturnMoneyStatusUtils.GetValue(EReturnMoneyStatus.UnRefund),
                                Status = EStatusUtils.GetValue(EStatus.Done)
                            });
                ...
                this.hlReturnOrderStatusPass.Attributes.Add("onclick", JsUtils.GetRedirectStringWithCheckBoxValueAndAlert(string.Format("{0}&ReturnOrder=True", PageUrl), "IDCollection", "IDCollection", "请选择返修单！", "您确定已收到退货了吗？"));
                this.hlReturnOrderStatusUnPass.Attributes.Add("onclick", JsUtils.GetRedirectStringWithCheckBoxValueAndAlert(string.Format("{0}&ReturnOrder=False", PageUrl), "IDCollection", "IDCollection", "请选择返修单！", "此操作将使返修单作废，确认吗？"));

                this.hlReturnMoneyStatusPass.Attributes.Add("onclick", JsUtils.GetRedirectStringWithCheckBoxValueAndAlert(string.Format("{0}&ReturnMoney=True", PageUrl), "IDCollection", "IDCollection", "请选择返修单！", "您确定买家已收到退款了吗？"));
                this.hlReturnMoneyStatusUnPass.Attributes.Add("onclick", JsUtils.GetRedirectStringWithCheckBoxValueAndAlert(string.Format("{0}&ReturnMoney=False", PageUrl), "IDCollection", "IDCollection", "请选择返修单！", "此操作将使返修单作废，确认吗？"));
        ...
        protected string GetSelectString()
        {
            ...
            {
                return DataProviderB2C.OrderItemReturnDAO.GetSelectString(base.PublishmentSystemID, EStatusUtils.GetValue(EStatus.Doing),
        ...
        protected string PageUrl
        {
            get
            {
                ...
                {
                    _pageUrl = BackgroundOrderReturnList1.GetRedirectUrl(base.PublishmentSystemID, this.ddlReturnType.SelectedValue, this.ddlAuditStatus.SelectedValue,
        ...
        {
            return PageUtils.GetB2CUrl(string.Format("background_orderReturnList1.aspx?PublishmentSystemID={0}", publishmentSystemID));
        }
        ...
        {
            return PageUtils.GetB2CUrl(string.Format("background_orderReturnList1.aspx?PublishmentSystemID={0}&returnType={1}&auditStatus={2}&returnOrderStatus={3}&returnMoneyStatus={4}&returnSN={5}&contact={6}&keyword={7}&page={8}", publishmentSystemID, returnType, auditStatus, returnOrderStatus, returnMoneyStatus, returnSN, contact, keyword, page));
        }
    }
}
