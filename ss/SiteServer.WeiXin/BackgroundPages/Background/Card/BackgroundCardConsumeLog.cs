...
namespace SiteServer.WeiXin.BackgroundPages
{
    public class BackgroundCardConsumeLog : BackgroundBasePageWX
    {
        ...
        {
            return PageUtils.GetWXUrl(string.Format("background_cardConsumeLog.aspx?PublishmentSystemID=" + publishmentSystemID + "&cardSN=" + cardSN + "&userName=" + userName + "&mobile=" + mobile));
        }
        ...
        public void Page_Load(object sender, EventArgs E)
        {
            ...
            if (!string.IsNullOrEmpty(base.Request.QueryString["Delete"])) 
            {
                ...
                if (list.Count > 0)
                {
                    try
                    {
                        ...
                        base.SuccessMessage("消费记录删除成功！");
                    }
                    catch (Exception ex)
                    {
                        base.FailMessage(ex, "消费记录删除失败！");
                    }
                }
            }
            ...
            this.spContents.SelectCommand = DataProviderWX.CardCashLogDAO.GetSelectString(base.PublishmentSystemID, ECashType.Consume, TranslateUtils.ToInt(base.Request.QueryString["cardID"]), base.Request.QueryString["cardSN"], base.Request.QueryString["userName"], base.Request.QueryString["mobile"]);
            ...
            if (!IsPostBack)
            {
                ...
                base.BreadCrumb(AppManager.WeiXin.LeftMenu.ID_Function, AppManager.WeiXin.LeftMenu.Function.ID_Card, "会员消费管理", AppManager.WeiXin.Permission.WebSite.Card);
                ...
                this.btnDelete.Attributes.Add("onclick", JsUtils.GetRedirectStringWithCheckBoxValueAndAlert(urlDelete, "IDCollection", "IDCollection", "请选择需要删除的消费记录", "此操作将删除所选消费记录，确认吗？"));
        ...
        private string PageUrl
        {
            get
            {
                ...
                {
                    this._pageUrl = PageUtils.GetWXUrl(string.Format("background_cardConsumeLog.aspx?PublishmentSystemID={0}&cardID={1}&cardSN={2}&userName={3}&mobile={4}", base.PublishmentSystemID,this.ddlCard.SelectedValue, this.tbCardSN.Text, this.tbUserName.Text, this.tbMobile.Text));
                }
