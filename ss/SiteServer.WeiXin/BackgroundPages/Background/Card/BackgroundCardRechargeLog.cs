...
namespace SiteServer.WeiXin.BackgroundPages
{
    public class BackgroundCardRechargeLog : BackgroundBasePageWX
    {
        ...
        {
            return PageUtils.GetWXUrl(string.Format("background_cardRechargeLog.aspx?PublishmentSystemID=" + publishmentSystemID + "&cardSN=" + cardSN + "&userName=" + userName + "&mobile=" + mobile));
        }
        ...
        public void Page_Load(object sender, EventArgs E)
        {
            ...
            if (!string.IsNullOrEmpty(base.Request.QueryString["Delete"])) 
            {
                ...
                if (list.Count > 0)
                {
                    try
                    {
                        ...
                        base.SuccessMessage("充值记录删除成功！");
                    }
                    catch (Exception ex)
                    {
                        base.FailMessage(ex, "充值记录删除失败！");
                    }
                }
            }
            ...
            this.spContents.SelectCommand = DataProviderWX.CardCashLogDAO.GetSelectString(base.PublishmentSystemID, ECashType.Recharge, TranslateUtils.ToInt(base.Request.QueryString["cardID"]), base.Request.QueryString["cardSN"], base.Request.QueryString["userName"], base.Request.QueryString["mobile"]);
            ...
            if (!IsPostBack)
            {
                ...
                base.BreadCrumb(AppManager.WeiXin.LeftMenu.ID_Function, AppManager.WeiXin.LeftMenu.Function.ID_Card, "会员卡充值管理", AppManager.WeiXin.Permission.WebSite.Card);
                ...
                string urlDelete = PageUtils.AddQueryString(BackgroundCardRechargeLog.GetRedirectUrl(base.PublishmentSystemID, this.tbCardSN.Text, this.tbUserName.Text, this.tbMobile.Text), "Delete", "True");
        ...
        private string PageUrl
        {
            get
            {
                ...
                {
                    this._pageUrl = PageUtils.GetWXUrl(string.Format("background_cardRechargeLog.aspx?PublishmentSystemID={0}&cardID={1}&cardSN={2}&userName={3}&mobile={4}", base.PublishmentSystemID,this.ddlCard.SelectedValue, this.tbCardSN.Text, this.tbUserName.Text, this.tbMobile.Text));
                }
