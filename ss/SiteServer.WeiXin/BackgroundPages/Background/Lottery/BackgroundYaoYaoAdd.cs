...
namespace SiteServer.WeiXin.BackgroundPages
{
    public class BackgroundYaoYaoAdd : BackgroundBasePageWX
	{
        ...
        {
            return PageUtils.GetWXUrl(string.Format("background_yaoYaoAdd.aspx?publishmentSystemID={0}&lotteryID={1}", publishmentSystemID, lotteryID));
        }
		...
		public void Page_Load(object sender, EventArgs E)
        {
			...
			if (!IsPostBack)
            {
                string pageTitle = this.lotteryID > 0 ? "编辑摇摇乐" : "添加摇摇乐";
                base.BreadCrumb(AppManager.WeiXin.LeftMenu.ID_Function, AppManager.WeiXin.LeftMenu.Function.ID_YaoYao, pageTitle, AppManager.WeiXin.Permission.WebSite.YaoYao);
                this.ltlPageTitle.Text = pageTitle;

                this.ltlImageUrl.Text = string.Format(@"<img id=""preview_imageUrl"" src=""{0}"" width=""370"" align=""middle"" />", LotteryManager.GetImageUrl(base.PublishmentSystemInfo, ELotteryType.YaoYao, string.Empty));
                this.ltlContentImageUrl.Text = string.Format(@"<img id=""preview_contentImageUrl"" src=""{0}"" width=""370"" align=""middle"" />", LotteryManager.GetContentImageUrl(base.PublishmentSystemInfo, ELotteryType.YaoYao, string.Empty));
                this.ltlAwardImageUrl.Text = string.Format(@"<img id=""preview_awardImageUrl"" src=""{0}"" width=""370"" align=""middle"" />", LotteryManager.GetAwardImageUrl(base.PublishmentSystemInfo, ELotteryType.YaoYao, string.Empty));
                this.ltlEndImageUrl.Text = string.Format(@"<img id=""preview_endImageUrl"" src=""{0}"" width=""370"" align=""middle"" />", LotteryManager.GetEndImageUrl(base.PublishmentSystemInfo, ELotteryType.YaoYao, string.Empty));
...
itemController.itemCount = {0};itemController.items = [{1}];", awardInfoList.Count, itemBuilder.ToString());
                ...
                this.btnReturn.Attributes.Add("onclick", string.Format(@"location.href=""{0}"";return false", BackgroundLottery.GetRedirectUrl(base.PublishmentSystemID, ELotteryType.YaoYao)));
			}
		}
		...
		public override void Submit_OnClick(object sender, EventArgs E)
		{
			if (base.Page.IsPostBack && base.Page.IsValid)
			{
                ...
                else if (selectedStep == 3)
                {
                    ...
                    {
                        base.FailMessage("摇摇乐保存失败，每人每天最多允许抽奖次数必须小于每人最多抽奖次数");
                        isItemReady = false;
                    }
                    ...
                    if (isItemReady)
                    {
                        ...
                        if (itemCount < 1)
                        {
                            base.FailMessage("摇摇乐保存失败，至少需要设置一个奖项");
                            isItemReady = false;
                        }
                        else
                        {
                            ...
                            {
                                base.FailMessage("摇摇乐保存失败，获奖概率之和必须在1%到100%之间");
                                isItemReady = false;
                            }
                ...
                else if (selectedStep == 5)
                {
                    ...
                    lotteryInfo.KeywordID = DataProviderWX.KeywordDAO.GetKeywordID(base.PublishmentSystemID, this.lotteryID > 0, this.tbKeywords.Text, EKeywordType.YaoYao, lotteryInfo.KeywordID);
                    ...
                    try
                    {
                        if (this.lotteryID > 0)
                        {
                            ...
                            LogUtils.AddLog(BaiRongDataProvider.AdministratorDAO.UserName, "修改摇摇乐", string.Format("摇摇乐:{0}", this.tbTitle.Text));
                            base.SuccessMessage("修改摇摇乐成功！");
                        }
                        else
                        {
                            ...
                            LogUtils.AddLog(BaiRongDataProvider.AdministratorDAO.UserName, "添加摇摇乐", string.Format("摇摇乐:{0}", this.tbTitle.Text));
                            base.SuccessMessage("添加摇摇乐成功！");
                        }

                        string redirectUrl = BackgroundLottery.GetRedirectUrl(base.PublishmentSystemID, ELotteryType.YaoYao);
                        base.AddWaitAndRedirectScript(redirectUrl);
                    }
                    catch (Exception ex)
                    {
                        base.FailMessage(ex, "摇摇乐设置失败！");
                    }
                }
			}
		}
