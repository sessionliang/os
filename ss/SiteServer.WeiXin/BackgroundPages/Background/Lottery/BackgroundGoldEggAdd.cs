...
namespace SiteServer.WeiXin.BackgroundPages
{
    public class BackgroundGoldEggAdd : BackgroundBasePageWX
	{
        ...
        {
            return PageUtils.GetWXUrl(string.Format("background_goldEggAdd.aspx?publishmentSystemID={0}&lotteryID={1}", publishmentSystemID, lotteryID));
        }
		...
		public void Page_Load(object sender, EventArgs E)
        {
			...
			if (!IsPostBack)
            {
                string pageTitle = this.lotteryID > 0 ? "编辑砸金蛋" : "添加砸金蛋";
                base.BreadCrumb(AppManager.WeiXin.LeftMenu.ID_Function, AppManager.WeiXin.LeftMenu.Function.ID_GoldEgg, pageTitle, AppManager.WeiXin.Permission.WebSite.GoldEgg);
                this.ltlPageTitle.Text = pageTitle;

                this.ltlImageUrl.Text = string.Format(@"<img id=""preview_imageUrl"" src=""{0}"" width=""370"" align=""middle"" />", LotteryManager.GetImageUrl(base.PublishmentSystemInfo, ELotteryType.GoldEgg, string.Empty));
                this.ltlContentImageUrl.Text = string.Format(@"<img id=""preview_contentImageUrl"" src=""{0}"" width=""370"" align=""middle"" />", LotteryManager.GetContentImageUrl(base.PublishmentSystemInfo, ELotteryType.GoldEgg, string.Empty));
                this.ltlAwardImageUrl.Text = string.Format(@"<img id=""preview_awardImageUrl"" src=""{0}"" width=""370"" align=""middle"" />", LotteryManager.GetAwardImageUrl(base.PublishmentSystemInfo, ELotteryType.GoldEgg, string.Empty));
                this.ltlEndImageUrl.Text = string.Format(@"<img id=""preview_endImageUrl"" src=""{0}"" width=""370"" align=""middle"" />", LotteryManager.GetEndImageUrl(base.PublishmentSystemInfo, ELotteryType.GoldEgg, string.Empty));
...
itemController.itemCount = {0};itemController.items = [{1}];", awardInfoList.Count, itemBuilder.ToString());
                ...
                this.btnReturn.Attributes.Add("onclick", string.Format(@"location.href=""{0}"";return false", BackgroundLottery.GetRedirectUrl(base.PublishmentSystemID, ELotteryType.GoldEgg)));
			}
		}
		...
		public override void Submit_OnClick(object sender, EventArgs E)
		{
			if (base.Page.IsPostBack && base.Page.IsValid)
			{
                ...
                else if (selectedStep == 3)
                {
                    ...
                    {
                        base.FailMessage("砸金蛋保存失败，每人每天最多允许抽奖次数必须小于每人最多抽奖次数");
                        isItemReady = false;
                    }
                    ...
                    if (isItemReady)
                    {
                        ...
                        if (itemCount < 1)
                        {
                            base.FailMessage("砸金蛋保存失败，至少需要设置一个奖项");
                            isItemReady = false;
                        }
                        else
                        {
                            ...
                            {
                                base.FailMessage("砸金蛋保存失败，获奖概率之和必须在1%到100%之间");
                                isItemReady = false;
                            }
                ...
                else if (selectedStep == 5)
                {
                    ...
                    lotteryInfo.KeywordID = DataProviderWX.KeywordDAO.GetKeywordID(base.PublishmentSystemID, this.lotteryID > 0, this.tbKeywords.Text, EKeywordType.GoldEgg, lotteryInfo.KeywordID);
                    ...
                    try
                    {
                        if (this.lotteryID > 0)
                        {
                            ...
                            LogUtils.AddLog(BaiRongDataProvider.AdministratorDAO.UserName, "修改砸金蛋", string.Format("砸金蛋:{0}", this.tbTitle.Text));
                            base.SuccessMessage("修改砸金蛋成功！");
                        }
                        else
                        {
                            ...
                            LogUtils.AddLog(BaiRongDataProvider.AdministratorDAO.UserName, "添加砸金蛋", string.Format("砸金蛋:{0}", this.tbTitle.Text));
                            base.SuccessMessage("添加砸金蛋成功！");
                        }

                        string redirectUrl = BackgroundLottery.GetRedirectUrl(base.PublishmentSystemID, ELotteryType.GoldEgg);
                        base.AddWaitAndRedirectScript(redirectUrl);
                    }
                    catch (Exception ex)
                    {
                        base.FailMessage(ex, "砸金蛋设置失败！");
                    }
                }
			}
		}
