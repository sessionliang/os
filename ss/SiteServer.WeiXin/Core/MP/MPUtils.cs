    ...
    public class MPUtils
    {
        ...
        public static string GetSitePreivewHtml(PublishmentSystemInfo publishmentSystemInfo, int nodeID, int contentID)
        {
            if (contentID > 0)
            {
                ...
                ContentInfo contentInfo = DataProvider.ContentDAO.GetContentInfoNotTrash(tableStyle, tableName, contentID);
                if (contentInfo != null)
                {
                    string pageUrl = PageUtilityWX.GetContentUrl(publishmentSystemInfo, contentInfo);
                    return string.Format(@"内容页：{0}&nbsp;<a href=""{1}"" target=""blank"">查看</a>", contentInfo.Title, pageUrl);
                }
            }
            else if (nodeID > 0)
            {
                ...
                if (nodeInfo != null)
                {
                    string nodeNames = NodeManager.GetNodeNameNavigation(publishmentSystemInfo.PublishmentSystemID, nodeID);
                    string pageUrl = PageUtilityWX.GetChannelUrl(publishmentSystemInfo, nodeInfo);
                    return string.Format(@"栏目页：{0}&nbsp;<a href=""{1}"" target=""blank"">查看</a>", nodeNames, pageUrl);
                }
            }
            return string.Empty;
        }
        ...
        public static string GetChannelOrContentSelectScript(PublishmentSystemInfo publishmentSystemInfo, int nodeID, int contentID)
        {
            ...
            if (nodeInfo != null)
            {
                if (contentID > 0)
                {
                    ...
                    {
                        string pageUrl = PageUtilityWX.GetContentUrl(publishmentSystemInfo, contentInfo);
                        return string.Format(@"contentSelect(""{0}"", ""{1}"", ""{2}"", ""{3}"")", contentInfo.Title, nodeID, contentID, pageUrl);
                    }
                }
                else if (nodeID > 0)
                {
                    string nodeNames = NodeManager.GetNodeNameNavigation(publishmentSystemInfo.PublishmentSystemID, nodeID);
                    string pageUrl = PageUtilityWX.GetChannelUrl(publishmentSystemInfo, nodeInfo);
                    return string.Format("selectChannel('{0}', '{1}', '{2}');", nodeNames, nodeID, pageUrl);
                }
            }
            return string.Empty;
        }
        ...
        public static string ParseEmotions(string content)
        {
            ...
            {
                foreach (var val in MPUtils.EmotionDictionary)
                {
                    content = StringUtils.Replace("&#47;" + val.Key, content, string.Format(@"<img src=""img/face/{0}.gif"" />", val.Value));
                }
            }
            return content;
        }
