    ...
    public class UEditorUploader
    {
        ...
        public Hashtable upFile(HttpContext cxt)
        {
            try
            {
                ...
                string localDirectoryPath = PathUtility.GetUploadDirectoryPath(this.publishmentSystemInfo, this.uploadType);
                string localFileName = PathUtility.GetUploadFileName(this.publishmentSystemInfo, originalName);
                string localFilePath = PathUtils.Combine(localDirectoryPath, localFileName);

                //格式验证
                if (!PathUtility.IsUploadExtenstionAllowed(this.uploadType, this.publishmentSystemInfo, currentType))
                {
                    state = "不允许的文件类型";
                }
                //大小验证
                if (!PathUtility.IsUploadSizeAllowed(this.uploadType, this.publishmentSystemInfo, uploadFile.ContentLength))
                {
                    state = "文件大小超出网站限制";
                }
                //保存图片
                if (state == "SUCCESS")
                {
                    ...
                    URL = PageUtility.GetPublishmentSystemUrlByPhysicalPath(this.publishmentSystemInfo, localFilePath);
                    //URL = pathbase + filename;
                    if (uploadType == EUploadType.Image)
                        //添加水印
                        FileUtility.AddWaterMark(this.publishmentSystemInfo, localFilePath);
                }
            }
            catch (Exception e)
            {
        ...
        public Hashtable upScrawl(HttpContext cxt, string base64Data)
        {
            ...
            try
            {
                ...
                string localDirectoryPath = PathUtility.GetUploadDirectoryPath(this.publishmentSystemInfo, fileExtension);
                string fileName = System.Guid.NewGuid() + fileExtension;
