...
{
    public class SurveyQuestionnaireDAO : DataProviderBase, ISurveyQuestionnaireDAO
    {
        public string TableName
        {
            get
            {
                return SurveyQuestionnaireInfo.TableName;
            }
        }

        public int Insert(SurveyQuestionnaireInfo info)
        {
            int contentID = 0;
        ...
        public void InsertWhithTrans(SurveyQuestionnaireInfo info, IDbTransaction trans)
        {
            IDbDataParameter[] parms = null;
            string SQL_INSERT = BaiRongDataProvider.TableStructureDAO.GetInsertSqlString(info.Attributes, TableName, out parms);
        ...
        public void Update(SurveyQuestionnaireInfo info)
        {
            IDbDataParameter[] parms = null;
            string SQL_INSERT = BaiRongDataProvider.TableStructureDAO.GetInsertSqlString(info.Attributes, TableName, out parms); ;
        ...
        public string GetSelectSqlString(int publishmentSystemID, List<int> channelIDList, string keyword, int searchDate, ETriState checkedState, ETriState channelState)
        {
            ...
            {
                checkString = "AND SurveyStatus = 'True'";
            }
            else if (checkedState == ETriState.False)
            {
                checkString = "AND SurveyStatus = 'False'";
            }
        ...
        public List<SurveyQuestionnaireInfo> GetInfoListChecked(int publishmentSystemID, int nodeID, int contentID)
        {
            List<SurveyQuestionnaireInfo> infoList = new List<SurveyQuestionnaireInfo>();
        ...
        public SurveyQuestionnaireInfo GetInfo(int publishmentSystemID, int nodeID, int contentID, int id)
        {
            SurveyQuestionnaireInfo info = new SurveyQuestionnaireInfo();
            string sqlString = string.Format("SELECT * FROM {4} WHERE PublishmentSystemID = {0} AND NodeID = {1} AND ContentID = {2} AND SQID = {3}", publishmentSystemID, nodeID, contentID, id, TableName);
            using (IDataReader rdr = this.ExecuteReader(sqlString))
            {
                while (rdr.Read())
                {
        ...
        {
            IDbDataParameter[] parms = null;
            string SQL_INSERT = string.Format("UPDATE {0} SET SurveyStatus='True',AdminName='{5}' WHERE PublishmentSystemID = {1} AND NodeID = {2} AND ContentID = {3} AND SQID = {4}", TableName, publishmentSystemID, nodeID, contentID, id,adminName);
