...
{
    public class BackgroundTrialReportContent : BackgroundBasePage
    {
        ...
        public void Page_Load(object sender, EventArgs E)
        {
            ...
            if (!IsPostBack)
            {
                base.BreadCrumb(AppManager.CMS.LeftMenu.ID_Function, AppManager.CMS.LeftMenu.Function.ID_Trial, "试用报告管理", AppManager.CMS.Permission.WebSite.Trial);
                ...
                this.spContents.SelectCommand = DataProvider.TrialReportDAO.GetSelectSqlString(base.PublishmentSystemID, channelIDList, this.Keyword.Text, TranslateUtils.ToInt(this.SearchDate.SelectedValue), checkedState, ETriState.False);
                ...
                if (nodeInfo.Additional.IsUseTrial)
                {
                    ...
                    this.btnDelete.Attributes.Add("onclick", JsUtils.GetRedirectStringWithCheckBoxValueAndAlert(PageUtils.GetCMSUrl(this.redirectStr + "&Delete=True"), "ContentIDCollection", "ContentIDCollection", "请选择需要删除的试用报告！", "此操作将删除所选试用报告，确认吗？"));
                }
                else
                {
                    this.phDelete.Visible = false;
                    this.alertsID.Text = "当前栏目未启用试用功能，试用功能不可用！";
                }
            }
        }
        ...
        void rptContents_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            {
                int id = TranslateUtils.EvalInt(e.Item.DataItem, TrialReportAttribute.TRID);
                ...
                string ipAddress = TranslateUtils.EvalString(e.Item.DataItem, TrialReportAttribute.IPAddress);
                DateTime addDate = TranslateUtils.EvalDateTime(e.Item.DataItem, TrialReportAttribute.AddDate);
                string content = TranslateUtils.EvalString(e.Item.DataItem, TrialReportAttribute.Description);
                string compositeScore = TranslateUtils.EvalString(e.Item.DataItem, TrialReportAttribute.CompositeScore);
                ...
                if (isChecked == false)
                {
                    ltlContent.Text += "&nbsp;<span style='color:#ff0000;text-decoration:none'>[未查看]</span>";
                }
                ...
                string showPopWinString = Modal.FunctionStyleValueShow.GetOpenWindowStringToTrialReport(base.PublishmentSystemID, this.nodeID, contentID, id);
        ...
        {
            return PageUtils.GetCMSUrl(string.Format("background_trialReportMainContent.aspx?PublishmentSystemID={0}&NodeID={1}", publishmentSystemID, nodeID));
        }
    }
}
