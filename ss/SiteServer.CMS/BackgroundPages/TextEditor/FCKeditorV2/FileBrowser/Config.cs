...
namespace SiteServer.CMS.BackgroundPages.FredCK.FCKeditorV2.FileBrowser
{
	public class Config : System.Web.UI.UserControl
	{
		...
		private void DefaultSettings() 
		{
            ...
            int publishmentSystemID = PublishmentSystemManager.GetPublishmentSystemIDByCache();
            if (publishmentSystemID != 0)
            {
                ...
                if (PublishmentSystemInfo != null)
                {
                    ...
                    {
                        UserFilesPath = string.Format("/{0}", PublishmentSystemInfo.Additional.FileUploadDirectoryName);
                    }
                    else
                    {
                        UserFilesPath = string.Format("/{0}/{1}", PublishmentSystemInfo.PublishmentSystemDir, PublishmentSystemInfo.Additional.FileUploadDirectoryName);
                    }
                }
            }
