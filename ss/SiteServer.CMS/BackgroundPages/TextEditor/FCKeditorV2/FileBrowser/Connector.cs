...
namespace SiteServer.CMS.BackgroundPages.FredCK.FCKeditorV2.FileBrowser
{
	public class Connector : FileWorkerBase
	{
		...
		private void GetFolders( XmlNode connectorNode, string resourceType, string currentFolder )
		{
            ...
            string sServerDir = PathUtility.MapPath(Config.PublishmentSystemInfo, "@" + currentFolder);
			...
			for ( int i = 0 ; i < aSubDirs.Length ; i++ )
			{
                ...
                if (
(aSubDirs[i].Name).ToUpper() != (DirectoryUtils.Bin.DirectoryName).ToUpper() && (aSubDirs[i].Name).ToUpper() != (DirectoryUtils.SiteFiles.DirectoryName).ToUpper() && (aSubDirs[i].Name).ToUpper() != (DirectoryUtils.Ask.DirectoryName).ToUpper() && (aSubDirs[i].Name).ToUpper() != (DirectoryUtils.LiveFiles.DirectoryName).ToUpper() && (aSubDirs[i].Name).ToUpper() != (DirectoryUtils.BBS.DirectoryName).ToUpper() && (aSubDirs[i].Name).ToUpper() != (DirectoryUtils.UserCenter.DirectoryName).ToUpper() &&  (aSubDirs[i].Name).ToUpper() != (DirectoryUtils.obj.DirectoryName).ToUpper() && (aSubDirs[i].Name).ToUpper() != (DirectoryUtils.SiteServer.DirectoryName).ToUpper())
                {
		...
		private void GetFiles( XmlNode connectorNode, string resourceType, string currentFolder )
		{
			...
			for ( int i = 0 ; i < aFiles.Length ; i++ )
			{
                    ...
                    if (aFiles[i].Name != DirectoryUtils.WebConfig.DirectoryName)
                {
		...
		{
            return PageUtility.GetPublishmentSystemUrl(Config.PublishmentSystemInfo, folderPath).TrimEnd('/') + "/";
