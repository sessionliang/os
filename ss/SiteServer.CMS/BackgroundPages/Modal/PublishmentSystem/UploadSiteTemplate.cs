...
{
	public class UploadSiteTemplate : BackgroundBasePage
	{
        ...
        public static string GetOpenWindowString()
        {
            ...
            return PageUtility.GetOpenWindowString("导入应用模板", "modal_uploadSiteTemplate.aspx", arguments, 460, 300);
        }
        ...
        public override void Submit_OnClick(object sender, EventArgs e)
        {
            ...
            if (isUpload)
            {
                if (myFile.PostedFile != null && "" != myFile.PostedFile.FileName)
                {
                    ...
                    {
                        base.FailMessage("应用模板压缩包为zip格式，请选择有效的文件上传");
                        return;
                    }
                    try
                    {
                        ...
                        {
                            base.FailMessage(string.Format("应用模板导入失败，文件夹{0}已存在", directoryName));
                            return;
                        }
                        string localFilePath = PathUtility.GetSiteTemplatesPath(directoryName + ".zip");
                        ...
                        JsUtils.OpenWindow.CloseModalPageAndRedirect(Page, PageUtils.GetSTLUrl("console_siteTemplate.aspx"));
                    }
                    catch (Exception ex)
                    {
            ...
            else
            {
                ...
                if (!StringUtils.EqualsIgnoreCase(sExt, ".zip"))
                {
                    base.FailMessage("应用模板压缩包为zip格式，请输入有效文件地址");
                    return;
                }
                string directoryName = PathUtils.GetFileNameWithoutExtension(this.tbDownloadUrl.Text);
                PageUtils.Redirect(Modal.ProgressBar.GetRedirectUrlStringWithSiteTemplateDownload(this.tbDownloadUrl.Text, directoryName));
            }
		}
