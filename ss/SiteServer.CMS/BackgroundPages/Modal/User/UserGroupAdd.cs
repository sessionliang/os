...
namespace SiteServer.CMS.BackgroundPages.Modal
{
    public class UserGroupAdd : BackgroundBasePage
	{
        ...
        public static string GetOpenWindowStringToEdit(int publishmentSystemID, int groupID, bool isCredits)
        {
            ...
            return PageUtility.GetOpenWindowString("修改用户组", "modal_userGroupAdd.aspx", arguments, 620, 550);
        }

        public static string GetOpenWindowStringToAdd(int publishmentSystemID, int groupID, bool isCredits)
        {
            NameValueCollection arguments = new NameValueCollection();
            arguments.Add("publishmentSystemID", publishmentSystemID.ToString());
            arguments.Add("GroupID", groupID.ToString());
            ...
            return PageUtility.GetOpenWindowString("添加用户组", "modal_userGroupAdd.aspx", arguments, 620, 550);
        }
        ...
        public override void Submit_OnClick(object sender, EventArgs e)
        {
            if (base.Page.IsPostBack && base.Page.IsValid)
            {
                if (!this.isAdd)
                {
                    ...
                    {
                        UserGroupInfo groupInfo = UserGroupManager.GetGroupInfo(base.PublishmentSystemInfo.GroupSN, this.groupID);
                        if (groupInfo.GroupName != this.GroupName.Text && BaiRongDataProvider.UserGroupDAO.IsExists(base.PublishmentSystemInfo.GroupSN, this.GroupName.Text))
                        {
                        ...
                        if (this.isCredits)
                        {
                            BaiRongDataProvider.UserGroupDAO.UpdateWithCredits(base.PublishmentSystemInfo.GroupSN, groupInfo, oldCreditsFrom, oldCreditsTo);
                        }
                        else
                        {
                ...
                {
                    if (BaiRongDataProvider.UserGroupDAO.IsExists(base.PublishmentSystemInfo.GroupSN, this.GroupName.Text))
                    {
                    ...
                    else
                    {
                        ...
                        if (this.isCredits && !BaiRongDataProvider.UserGroupDAO.IsCreditsValid(base.PublishmentSystemInfo.GroupSN, creditsFrom, creditsTo))
                        {
                            base.FailMessage("用户组添加失败，积分范围必须在已有的某个用户组积分范围之内！");
                            return;
                        }

                        UserGroupInfo groupInfo = new UserGroupInfo(0, base.PublishmentSystemInfo.GroupSN, this.GroupName.Text, this.isCredits ? EUserGroupType.Credits : EUserGroupType.Specials, creditsFrom, creditsTo, TranslateUtils.ToInt(this.Stars.Text), this.Color.Text, string.Empty);
