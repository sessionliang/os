...
namespace SiteServer.CMS.BackgroundPages.Modal
{
    public class SelectImage : BackgroundBasePage
    {
        ...
        //限制限制文件夹路径最高路径
        private const string topPath = "@/upload/images";
        ...
        private string GetLinkUrl(string path)
        {
            ...
            if (!DirectoryUtils.IsInDirectory(topPath, path))
                path = topPath;
            return PageUtils.GetCMSUrl(string.Format("modal_selectImage.aspx?PublishmentSystemID={0}&RootPath={1}&CurrentRootPath={2}&TextBoxClientID={3}", base.PublishmentSystemID, this.rootPath, path, this.textBoxClientID));
        }
        ...
        public static string GetOpenWindowString(PublishmentSystemInfo publishmentSystemInfo, string textBoxClientID)
        {
            ...
            arguments.Add("CurrentRootPath", topPath);
            arguments.Add("TextBoxClientID", textBoxClientID);
            return PageUtility.GetOpenWindowString("选择图片", "modal_selectImage.aspx", arguments, 550, 480, true);
        }
        ...
        public void Page_Load(object sender, EventArgs E)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                this.hlUploadLink.Attributes.Add("onclick", Modal.UploadImageSingle.GetOpenWindowStringToList(base.PublishmentSystemID, this.currentRootPath));
...
", backgroundImageUrl, textBoxUrl, linkUrl, fileImageUrl, imageStyleAttributes, StringUtils.MaxLengthText(fileInfo.Name, 7));
