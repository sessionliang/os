...
{
    public class ConsoleSiteDownloadAnalysis : BackgroundBasePage
    {
        //默认展示前10条数据
        ...
        public void SetYHashtable(int publishemtSystemID, int value)
        {
            ...
            if (!YHashtableDownload.ContainsKey(publishemtSystemID))
            {
                YHashtableDownload.Add(publishemtSystemID, value);
            }
            else
            {
                int num = TranslateUtils.ToInt(YHashtableDownload[publishemtSystemID].ToString());
                YHashtableDownload[publishemtSystemID] = num + value;
            }
        ...
        public void Page_Load(object sender, EventArgs E)
        {
            ...
            if (!IsPostBack)
            {
                base.BreadCrumbConsole(AppManager.Platform.TopMenu.ID_Statistics, "文件下载量统计", AppManager.Platform.Permission.Platform_Statistics);

                this.lbMore.Attributes.Add("href", ConsoleSiteDownloadAnalysisAll.GetRedirectUrl(this.PageUrl));
        ...
        public void BindGrid()
        {
            ...
            while (ie.MoveNext())
            {
                ...
                //y轴信息
                SetYHashtable(key
                                           , CountManager.GetCount(AppManager.CMS.AppID, publishmentSystemInfo.AuxiliaryTableForContent, publishmentSystemInfo.PublishmentSystemID, ECountType.Download)
                                           );
        ...
        private string PageUrl
        {
            get
            {
                ...
                {
                    _pageUrl = PageUtils.GetCMSUrl(string.Format("console_siteDownloadAnalysis.aspx"));
                }
                return _pageUrl;
            }
        }
