...
(function (ss) {
    var validater = {
        event_observe: function (attributeName, eventName, handler) {
        ...
        checkAttributeValueByParam: function (attributeName, isRequire, value, errorMessage, displayName, minNum, maxNum, validateType, regExp) {
            try {
        ...
        checkAttributeValue: function (event) {
            ...
            return window.ss.extendjs.validater.checkAttributeValueByParam(attributeName, isRequire, value, errorMessage, displayName, minNum, maxNum, validateType, regExp);
        },

        checkAttributeValueById: function (attributeName) {
        ...
        checkAttributeValueByName: function (attributeName) {
            var obj = document.getElementById(attributeName);
            if (obj) {
                var isRequire = obj.getAttribute("isRequire");
        ...
        checkFormValueById: function (formId) {
            var isChecked = true;
                ...
                for (i = 0; i < myArray.length; i++) {
                    ...
                    if (item.getAttribute("isValidate") == "true") {
                        if (item.getAttribute("islistitem") == "true") {
                            if (window.ss.extendjs.validater.checkAttributeValueByName(item.getAttribute("id")) == false) {
                                isChecked = false;
                            }
                        }
                        else {
                            if (window.ss.extendjs.validater.checkAttributeValueById(item.getAttribute("id")) == false) {
                                isChecked = false;
                            }
                        }
                    }
    ...
    window.ss.extendjs.validater = validater;
    //ss中负责校验的方法
    window.ss.extendjs.checkFormValueById = function (formId) {
        return validater.checkAttributeValueById.call(window.ss.extendjs.validater, formId);
    };
    window.ss.extendjs.event_observe = function (attributeName, eventName, handler) {
        return validater.event_observe.call(window.ss.extendjs.validater, attributeName, eventName, handler);
    };
})(ss);
